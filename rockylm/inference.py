...
class RockyInference:
    def __init__(self, checkpoint_path, tokenizer_path, device="cpu"):
        ...
        if os.path.exists(config_path):
            ...
            self.config = RockyConfig(**{k: v for k, v in ckpt["config"].items() if k in valid_fields})
        else:
            print("Warning: No config found, using defaults")
            self.config = RockyConfig()

        self.model = RockyLM(self.config).to(self.device)
        ...
        print(f"RockyLM loaded: {total/1e6:.1f}M params")
...
def main():
    import argparse
    p = argparse.ArgumentParser(description="Chat with Rocky")
    p.add_argument("--checkpoint", default="checkpoints/best_model.pt")
    p.add_argument("--tokenizer", default="data/tokenizer.json")
    p.add_argument("--device", default="cpu")
    p.add_argument("--prompt", "-p", help="Single prompt mode: ask one question and exit")
    p.add_argument("--speak", action="store_true", help="Speak Rocky's replies aloud")
    p.add_argument("--tts-backend", default="peon", choices=["peon", "xtts"],
                   help="Voice backend: 'peon' (recorded clips) or 'xtts' (neural clone)")
    args = p.parse_args()
    ...
    def say(text):
        if args.speak and text:
            try:
                ...
                speak(text, backend=args.tts_backend)
            except Exception as e:
                print(f"(tts unavailable: {e})")

    engine = RockyInference(args.checkpoint, args.tokenizer, args.device)

    if args.prompt:
        text = engine.chat_completion([{"role": "user", "content": args.prompt}])["choices"][0]["message"]["content"]
        print(text)
        say(text)
        return

    print("\nRocky Chat (type 'quit' to exit)")
    while True:
        inp = input("\nYou> ").strip()
