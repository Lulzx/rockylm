"""
Export RockyLM to ONNX format for browser inference via onnxruntime-web.
...
def export_onnx(checkpoint_path, tokenizer_path, output_path, quantize=True, push=False):
    ...
    if os.path.exists(config_json):
        ...
        config = RockyConfig(**{k: v for k, v in ckpt["config"].items() if k in valid_fields})
    else:
        config = RockyConfig()
    ...
    print(f"RockyLM: {total:,} params ({total/1e6:.1f}M)")
...
def main():
    parser = argparse.ArgumentParser(description="Export RockyLM to ONNX")
