"""
Export RockyLM to HuggingFace standard format.
...
def export_and_push(checkpoint_path, tokenizer_path, repo_id, token, local_dir="hf_export"):
    ...
    # 5. rocky.png — asset
    asset_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "assets", "rocky.png")
    if os.path.exists(asset_path):
        asset_dir = os.path.join(local_dir, "assets")
        os.makedirs(asset_dir, exist_ok=True)
        import shutil
        shutil.copy2(asset_path, os.path.join(asset_dir, "rocky.png"))
        print(f"  Saved assets/rocky.png")
...
def main():
    parser = argparse.ArgumentParser(description="Export RockyLM to HuggingFace format")
