"""Generate the RockyLM Colab training notebook."""
...
FILES = [
    ("config.py",    "rockylm/config.py"),
    ("model.py",     "rockylm/model.py"),
    ("dataset.py",   "rockylm/dataset.py"),
    ("train.py",     "rockylm/train.py"),
    ("inference.py", "rockylm/inference.py"),
]
...
def build():
    ...
    cells.append(code(
        ...
        "if os.path.exists('/content/rocky'):\n"
        "    shutil.rmtree('/content/rocky')\n"
        "os.makedirs('/content/rocky')\n"
        "os.chdir('/content/rocky')\n"
    ...
    cells.append(code(
        ...
        "HF_DATASET = 'arman-bd/rockylm-60k-generic'\n"
    ...
    cells.append(code(
        "from inference import RockyInference\n"
        "import torch\n"
        "\n"
        "engine = RockyInference(\n"
        ...
        "    ('goodnight rocky',               'night'),\n"
        "]\n"
        "\n"
        "print(f'{\"Topic\":<12s}  {\"You\":<35s}  Rocky')\n"
        "print('=' * 100)\n"
    ...
    cells.append(code(
        ...
        "HF_REPO = os.environ.get('HF_REPO', 'arman-bd/rockylm-9M')  # Or change this\n"
        "\n"
        "# Load checkpoint\n"
        ...
        "valid_fields = {f.name for f in RockyConfig.__dataclass_fields__.values()}\n"
        "config = RockyConfig(**{k: v for k, v in cfg.items() if k in valid_fields})\n"
        "model = RockyLM(config)\n"
    ...
    cells.append(code(
        "import os\n"
        "\n"
        "!cd /content && tar czf rockylm.tar.gz \\\n"
        "    rocky/checkpoints/best_model.pt \\\n"
        "    rocky/checkpoints/config.json \\\n"
        "    rocky/data/tokenizer.json \\\n"
        "    rocky/model.py \\\n"
        "    rocky/config.py \\\n"
        "    rocky/inference.py \\\n"
        "    rocky/hf_export/model.onnx\n"
        "\n"
        "sz = os.path.getsize('/content/rockylm.tar.gz') / 1e6\n"
        "print(f'Package: /content/rockylm.tar.gz ({sz:.1f} MB)')\n"
        "\n"
        "try:\n"
        "    from google.colab import files\n"
        "    files.download('/content/rockylm.tar.gz')\n"
        "except ImportError:\n"
    ...
    return {
        ...
        "metadata": {
            "colab": {"provenance": [], "gpuType": "T4", "name": "RockyLM — Train"},
...
def build_use():
    """Build the use_rockylm notebook — download model from HF and chat."""
    cells = []

    cells.append(md(
        "# RockyLM — Chat with a Fish\n"
        "\n"
        "Download a pre-trained 9M parameter fish LLM and chat with it. Just run all cells.\n"
        "\n"
        "**Model:** [arman-bd/rockylm-9M](https://huggingface.co/arman-bd/rockylm-9M)"
    ))
    ...
    cells.append(code(
        ...
        "import os, shutil\n"
        "if os.path.exists('/content/rocky'): shutil.rmtree('/content/rocky')\n"
        "os.makedirs('/content/rocky'); os.chdir('/content/rocky')\n"
        "\n"
        "from huggingface_hub import snapshot_download\n"
        "snapshot_download(repo_id='arman-bd/rockylm-9M', local_dir='.')\n"
    ...
    cells.append(code(
        ...
        "import torch\n"
        "\n"
        "engine = RockyInference('pytorch_model.bin', 'tokenizer.json',\n"
        "                        device='cuda' if torch.cuda.is_available() else 'cpu')\n"
        "\n"
        "def chat(prompt):\n"
        ...
        "\n"
        "# Quick test\n"
        "for p in ['hi rocky', 'are you hungry', 'tell me a joke', 'what is the internet', 'goodnight rocky']:\n"
        "    print(f'You> {p}\\nRocky> {chat(p)}\\n')"
    ))
    ...
    cells.append(code(
        ...
        "        print('Rocky> bye. i will continue being a fish.'); break\n"
        "    print(f'Rocky> {chat(p)}\\n')"
    ))

    return {
        "nbformat": 4, "nbformat_minor": 0,
        "metadata": {
            "colab": {"provenance": [], "name": "RockyLM — Chat"},
            "kernelspec": {"name": "python3", "display_name": "Python 3"},
            "language_info": {"name": "python"},
        },
        "cells": cells,
...
if __name__ == "__main__":
    write_notebook(build(), "train_rockylm.ipynb")
    write_notebook(build_use(), "use_rockylm.ipynb")
